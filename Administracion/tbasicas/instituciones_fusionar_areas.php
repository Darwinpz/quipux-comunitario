...
$inst_adscrita = ($_POST["chk_inst_adscrita"]==1) ? $inst_origen : $inst_destino;
...
if ($depe_origen == 0) {
    $sql = "select depe_codi from dependencia where inst_codi=$inst_origen limit 1000";
} else {
    ...
    $sql = "select depe_codi from dependencia where depe_codi in ($lista_areas) and inst_codi=$inst_origen limit 1000";
}
...
$contador = 0;
while($rs && !$rs->EOF) {
    $record["depe_codi"]     = trim($rs->fields["DEPE_CODI"]);
    ...
    if ($ok != 1) {
        ...
        die ("Error-No se pudo mover el &aacute;rea ".trim($rs->fields["DEPE_CODI"]));
    }
    ++$contador;
    $rs->MoveNext();
}
...
if ($depe_origen != 0) {
    $sql = "update dependencia set depe_codi_padre=depe_codi where depe_codi=$depe_origen";
    $ok = $db->query($sql);
    if ($ok != 1) {
        $db->conn->RollbackTrans();
        die ("Error-No se pudo quitar referencia con el área padre");
    }
}
