...
if ($depe_origen == 0) {
    $sql = "select lista_codi from lista where inst_codi=$inst_origen limit 1000";
} else {
    ...
    $sql = "select lista_codi from lista
            where usua_codi in (select usua_codi from usuarios where depe_codi in ($lista_areas))
            and inst_codi=$inst_origen limit 1000";
}
...
$contador = 0;
while($rs && !$rs->EOF) {
    $record["lista_codi"] = trim($rs->fields["LISTA_CODI"]);
    $record["inst_codi"] = $inst_destino;
    $ok = $db->conn->Replace("lista", $record, "lista_codi", false,false,true,false);
    if ($ok != 1) {
        $db->conn->RollbackTrans();
        die ("Error-No se pudo mover la lista ".trim($rs->fields["LISTA_CODI"]));
    }
    ++$contador;
    $rs->MoveNext();
}
