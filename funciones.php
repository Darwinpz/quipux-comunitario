...
echo "<script>function firmaEC(){window.open('http://www.firmadigital.gob.ec/descargar-firmaec/','popup','width=700,height=800,scrollbars=1')}</script>";
$html="";
$html.="<$centrado>";
$html.="<table>";
$html.="<tr><td><br><center><b><font size='2' color='#F09524'><i>Para continuar, asegúrese de tener instalado la aplicación FirmaEC, descargue el instalador </i></font><a href='#' onclick='firmaEC();'><font size='2' color='blue'><i>aquí</i></font></a></b></center></td></tr>";
$html.="</table>";
$html.="</$centrado>";
return $html;
...
/*******************************************************************************
** Me envía un email si se detectó un intento de ataque                       **
*******************************************************************************/
function enviar_mail_intento_ataque($asunto, $cadena) {
    $mail_destino = "Soporte <soporte@informatica.gob.ec>";
    $dir_cliente = $_SERVER['HTTP_X_FORWARDED_FOR'] . " - " . $_SERVER['HTTP_CLIENT_IP'] . " - " . $_SERVER['REMOTE_ADDR'];
    $cadena = str_ireplace(array("<",">","javascript","script"), array("&laquo;","&raquo;","jaiba_street","street"), $cadena);
    $cadena = str_ireplace(array('&laquo;font color="red"&raquo;',"&laquo;/font&raquo;"), array('<font color="red">',"</font>"), $cadena);

    $mensaje = '<center><b>'.$asunto.'</b></center><br>
        <table width="100%" border="1">
            <tr><td>Fecha</td><td>'.date("Y-m-d H:i:s").'</td></tr>
            <tr><td>Cadena</td><td>'.$cadena.'</td></tr>
            <tr><td>Usuario</td><td>'.$_SESSION["usua_codi"].' - '.$_SESSION["krd"].' - '.$_SESSION["usua_nomb"].'</td></tr>
            <tr><td>Cliente</td><td>'.$dir_cliente.'</td></tr>
            <tr><td>Servidor</td><td>'.$_SERVER["SERVER_ADDR"].'</td></tr>
            <tr><td>PHP_SELF</td><td>'.$_SERVER["PHP_SELF"].'</td></tr>
            <tr><td>HTTP_REFERER</td><td>'.$_SERVER["HTTP_REFERER"].'</td></tr>
        </table>';
    $variables = array("_SESSION", "_GET", "_POST", "_SERVER");

    foreach ($variables as $tipo_variable) {
        $mensaje .= '<br><center><b>VARIABLES $'.$tipo_variable.'</b></center><br>
            <table width="100%" border="1">';
        global $$tipo_variable;
        foreach ($$tipo_variable as $key => $value) {
            if ($tipo_variable!="_SERVER" or substr($key,0,3)!="DB_") { //Para que no envie las contraseñas de la BDD almacenadas en variables $_SERVER
                $mensaje .= '
                    <tr><td>'.$key.'&nbsp;</td><td>'.str_ireplace(array("<",">","javascript","script"), array("&laquo;","&raquo;","jaiba_street","street"), $value).'&nbsp;</td></tr>';
            }
        }
        $mensaje .= '</table>';
