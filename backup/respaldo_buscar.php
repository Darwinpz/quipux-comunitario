...
**/
  ...
  $usr_codi_actual = $_SESSION["usua_codi"];
...
switch ($txt_tipo_lista) {
    case 1: //Solicitudes del usuario logueado
        ...
        break;
    case 2: //Solicitudes por autorizar de la institución que puede ver quien autoriza       
            $isql .= " and us.inst_codi=$usr_inst_actual and estsol.est_codi=2
            and us.depe_codi in (select depe_codi from permiso_usuario_dep
            where id_permiso = 33
            and usua_codi = $usr_codi_actual)";
        break;
    case 3: //Listado de Solicitudes de la institución que puede ver quien autoriza      
            $isql .= " and  us.inst_codi=$usr_inst_actual and estsol.est_codi<>1
            and us.depe_codi in (select depe_codi from permiso_usuario_dep
            where id_permiso = 33
            and usua_codi = $usr_codi_actual)";       
        break;
    case 6: //Listado de Solicitudes por enviar de la Institución actual y del usuario que crea la solicitud
            $isql .= " and  us.inst_codi=$usr_inst_actual and estsol.est_codi=1
            and usua_codi_accion = $usr_codi_actual"; 
        break;
    case 7: //Listado de Solicitudes de la Institución actual
            $isql .= " and  us.inst_codi=$usr_inst_actual and estsol.est_codi<>1"; //Estado diferentes a edicion
        break;
    case 9: //Listado de Solicitudes por enviar de la Institución seleccionada y del usuario que crea la solicitud
            if($codi_inst == 0)
                $isql .= " and estsol.est_codi=1 and usua_codi_accion = $usr_codi_actual";
            else
                $isql .= " and us.inst_codi=$codi_inst and estsol.est_codi=1 and usua_codi_accion = $usr_codi_actual";

        break;
     case 10: //Listado de Solicitudes de la Institución seleccionada
            if($codi_inst == 0)
                $isql .= " and estsol.est_codi<>1";
            else
                $isql .= " and us.inst_codi=$codi_inst and estsol.est_codi<>1";
        break;
     case 11: //Listado de Solicitudes autorizadas de la Institución seleccionada
         if($codi_inst == 0)
             $isql .= " and estsol.est_codi=3";
         else
             $isql .= " and  us.inst_codi=$codi_inst and estsol.est_codi=3";            
        break;
    default:
        break;
}
    ...
    if ($txt_nombre != "")
        $isql .= ' and ((' . buscar_cadena($txt_nombre, "us.usua_nomb") . ') or (' . buscar_cadena($txt_nombre, "us.usua_apellido") . ') or (' . buscar_cadena($txt_nombre, "us.usua_cedula") . '))';
